...
Follow [livox_ros2_driver Installation](https://github.com/Livox-SDK/livox_ros2_driver).

You can also use the one I modified [livox_ros2_driver](https://github.com/Ericsii/livox_ros2_driver/tree/feature/use-standard-unit)
...
Connect to your PC to Livox LiDAR by following  [Livox-ros-driver2 installation](https://github.com/Livox-SDK/livox_ros2_driver), then
```bash
cd <ros2_ws>
. install/setup.bash # use setup.zsh if use zsh
...
```bash
ros2 launch livox_ros2_driver msg_MID360_launch.py
```

- For livox serials, FAST-LIO only support the data collected by the ``` livox_lidar_msg.launch ``` since only its ``` livox_ros2_driver/CustomMsg ``` data structure produces the timestamp of each LiDAR point which is very important for the motion undistortion. ``` livox_lidar.launch ``` can not produce it right now.
- If you want to change the frame rate, please modify the **publish_freq** parameter in the [livox_lidar_msg.launch](https://github.com/Livox-SDK/livox_ros_driver/blob/master/livox_ros2_driver/launch/livox_lidar_msg.launch) of [Livox-ros-driver](https://github.com/Livox-SDK/livox_ros2_driver) before make the livox_ros_driver pakage.
